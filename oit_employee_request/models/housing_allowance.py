...
class housing_allowance(models.Model):
    ...
    date_last_bouns_hous = fields.Date(string="Date Last Housing Allowance", required=False,related="employee_id.date_last_bouns_hous", )



    housing_allowance_value = fields.Float(string="Housing Allowance Value",readonly=True, states={'draft': [('readonly', False)]},)
    start_housing_allowance = fields.Date(string="Start Housing Allowance",readonly=True, states={'draft': [('readonly', False)]},)
    end_housing_allowance = fields.Date(string="End Housing Allowance",readonly=True, states={'draft': [('readonly', False)]},)
    ...
    def confirm_entry(self):
        ...
        vals = []
        price = self.housing_allowance_value
        if not price:
            raise exceptions.ValidationError("Can't Conform with 0.0 Amount")
        ref = 'Housing Request# ' + self.name
        if price:
            vals.append((0, 0, {
        ...
        move = account_move_obj.create(
            {'date':self.request_date, 'ref': ref, 'journal_id': self.journal_id.id, 'move_type': 'entry', 'line_ids': vals})
        print("move ==> ", move)
    ...
    def pay_entry(self):
        ...
        move = account_move_obj.create(
            {'date':self.request_date, 'ref': ref, 'journal_id': self.pay_journal_id.id, 'move_type': 'entry', 'line_ids': vals})
        print("move ==> ", move)
        move.sudo().action_post()
        self.move_ids = [(4, move.id)]
    ...
    def create(self, values):
        values['name'] = self.env['ir.sequence'].next_by_code('housing.order') or _('New')
        return super(housing_allowance, self).create(values)
