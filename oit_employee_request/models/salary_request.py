...
class salary_request(models.Model):
    _name = "salary.request"
    _inherit = ['mail.thread', 'mail.activity.mixin']

    def unlink(self):
        if any(rec.state != 'draft' for rec in self):
            raise exceptions.ValidationError(_('You cannot delete records if they are in non-draft state'))
        return super(salary_request, self).unlink()

    name = fields.Char(string="Request", readonly=True,copy=False)
    employee_id = fields.Many2one(comodel_name="hr.employee",required=True, readonly=True, states={'draft': [('readonly', False)]},  string="Employee")
    ...
    salary_value = fields.Float(string="Salary Value",readonly=True, states={'draft': [('readonly', False)]}, )
    month_due = fields.Date(string="Month Due",readonly=True, states={'draft': [('readonly', False)]}, )
    ...
    def create(self, vals):
        print("values ==> ", vals)
        vals['name'] = self.env['ir.sequence'].next_by_code('salary.order') or _('New')
        print("values2 ==> ",vals)
        return super(salary_request, self).create(vals)
